...
atribuirRevisores().then(() => {
    ...
    commits.forEach(commit => {
        ...
        console.log(`autor: ${commit.author_email} \trevisores: ${JSON.stringify(commit.revisores)} \thistorico: ${JSON.stringify(commit.historico)}`);
    });
    console.log('---------------------------------');
    console.log('COUNTS:');
...
function assertCommit(commit) {
    ...
    assert(commit.revisores.length > 0);
